...
- **Gateway 重启后的工具凭据续接**：Channel 与 Gateway 是两个进程；Channel 的 CLI 凭据不会天然出现在 Gateway。启用 `gateway_url` 时，`_GatewayRouteProvider.ensure()` 在每次路由（包括 socket 缓存命中）前向 loopback-only `POST /feishu/runtime-config` 重送 app id/secret，Gateway 仅写入本进程环境、不落盘，故 Gateway 单独重启后下一条飞书消息即可恢复 workspace Feishu tools。404 兼容旧 Gateway 并继续路由；其他错误 fail closed 后由既有 fallback 处理。该内部端点刻意不进 OpenAPI。
- 用户白名单：`--allowed-user-ids` 参数或 `None`（不限制）
...
- **交互卡片回调上下文与确定性分发**：Haitun workspace 的 `feishu_message_send_card` 在发送成功后按 `message_id` 把原始卡片、发送方 Session / open_id、接收目标、业务上下文和 `action_id -> handler` 映射写入 AppData 的 v2 snapshot。`_card_action.handle_card_action` 原子领取 snapshot 后，直接用 Python 比较用户选择与正确答案并发送固定反馈；随后用 channel 已有的 tenant token 直接 `POST` Base 记录，再按返回的 `record_id` `GET` 回读确认。整个答题点击路径不调 Session/LLM，且写入不自动重试，避免超时后重复行。配置了非空 handler 映射时只允许 canonical action ID 精确命中；未知 action 或 snapshot 异常时 fail closed。首个回调把 snapshot 原子改名为持久 `.consumed` tombstone，后续重复回调直接忽略。Channel 的自定义 `appdata` 必须与 Gateway/workspace tool 解析到同一根（推荐统一设置 `PSI_APPDATA`）。
