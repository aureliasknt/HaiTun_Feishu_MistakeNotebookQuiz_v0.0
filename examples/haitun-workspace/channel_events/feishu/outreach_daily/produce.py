...
def _is_paused(user: dict[str, Any]) -> bool:
    """True when this target is paused, so the daily send must skip them.

    **(Deliberate duplication.)** The owning definition is
    ``tools/_outreach_confirm.is_paused``, and this is a copy rather than an import
    because ``produce.py`` is ``exec``-ed as a standalone module by
    ``channel/_event_defs._exec_py_module`` — no package, and ``tools/`` is not on
    ``sys.path``. Importing it would mean this producer pushes ``sys.path`` at
    module scope, which is worse than four lines restated next to a pointer.

    Keep the two in step: only the explicit ``paused`` value pauses, so a missing or
    unrecognised ``status`` stays active and no existing row changes behaviour.
    """
    return str(user.get("status") or "active").strip().casefold() == "paused"
...
async def produce(ctx: Any) -> None:
    while True:
        try:
            ...
            # Paused targets are dropped here, before the envelope is built: the
            # agent receives the send list and would otherwise DM somebody who is
            # switched off (see _is_paused).
            open_ids = [
                u.get("open_id")
                for u in state.get("users") or []
                if isinstance(u, dict) and isinstance(u.get("open_id"), str) and not _is_paused(u)
            ]
