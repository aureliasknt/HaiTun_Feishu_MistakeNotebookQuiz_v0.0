---
name: feishu-chat
description: 飞书群（chat）接口表 —— 建群/拉人/踢人、群设置、禁言、转让群主、解散群、群菜单、群标签页。用 feishu_api 按表调用。注意「把某人加进来」若指的是智能体科普名单而非群，用 outreach-confirmation-card；两种读法都成立时先反问。
---
...
**「把 @某人 加进来」不一定是拉人进群。** 它同样可以指加进智能体科普名单（`outreach-confirmation-card`
的 `outreach_target_add`），两者后果完全不同：一个把人拉进群，一个让人开始每天收到机器人私聊。
提到 群 / 拉进群 / 具体群名 / `oc_...` 才走本表；提到 名单 / 科普 / 智能体 / outreach 走那个能力包；
**两种都说得通就反问一句**，别猜。私聊里尤其要当心：DM 的 `chat_id` 是这次私聊本身，不是可拉人的群。
「别再发给他了」「暂停某人」也不是踢出群，是 `outreach_target_pause`。
